...
personasModel.getUserInner = (callback) => {
  if (connection) {
    connection.query('SELECT personas.primer_nombre, personas.segundo_nombre, personas.a_paterno, personas.a_materno, personas.sexo, personas.fecha_nacimiento, grado_estudios.estudios, personas.escuela, ocupacion.ocupacion, estado_civil.estado, personas.lugar_origen, personas.telefono, personas.fecha_registro, personas.id_familia, familias.nombre_familia, barrios_colonias.colonia, familias.direccion, familias.calle_lateral_derecha, familias.calle_lateral_izquierda FROM personas INNER JOIN familias ON id_persona = id_persona INNER JOIN barrios_colonias ON familias.id_colonia = barrios_colonias.id_colonia INNER JOIN grado_estudios ON personas.id_grado_estudios = grado_estudios.id_grado_estudios INNER JOIN ocupacion ON personas.id_ocupacion = ocupacion.id_ocupacion INNER JOIN estado_civil ON personas.id_estado_civil = estado_civil.id_estado_civil;',
      (err, rows) => {
        if (err) {
          throw err
        } else {
