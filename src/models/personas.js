...
personasModel.getUser = (callback) => {
  if (connection) {
    connection.query('SELECT * FROM persona WHERE estatus = 1 ORDER BY id_persona DESC',
      (err, rows) => {
        if (err) {
          throw err
        } else {
...
personasModel.getPersonasById = (userId, callback) => {
  if (connection) {
    connection.query('SELECT * FROM persona WHERE `estatus` = 1 AND `id_persona` = ? ORDER BY id_persona DESC',
      (userId), (err, rows) => {
        if (err) {
          throw err
        } else {
...
personasModel.insertPersona = (userData, callback) => {
  if (connection) {
    connection.query('INSERT INTO persona SET ?', userData,
      (err, result) => {
        if (err) {
          throw err
        } else {
...
personasModel.deleteUser = (id, callback) => {
  if (connection) {
    let sqlExists =
      `SELECT * FROM persona WHERE id_persona = ${connection.escape(id)}`
    connection.query(sqlExists, (_err, row) => {
      if (row) {
        let sql = `UPDATE persona SET estatus = 0 WHERE id_persona = ${connection.escape(id)}`
        connection.query(sql, (err, _result) => {
          if (err) {
            throw err
          } else {
